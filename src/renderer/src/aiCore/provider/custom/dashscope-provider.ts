...
class DashScopeImageModel implements ImageModelV3 {
  ...
  async doGenerate(options: ImageModelV3CallOptions): Promise<Awaited<ReturnType<ImageModelV3['doGenerate']>>> {
    ...
    if (imageUrls.length === 0) throw new Error('DashScope image response contained no images')
...
export function createDashScope(options: DashScopeProviderSettings = {}): DashScopeProvider {
  ...
  // Preserve the renderer transport: Electron's session guard owns the live
  // allowlist for all requests, including generated-image downloads.
  const resolveApiKey = () =>
  ...
  provider.imageModel = (modelId: string) =>
    /^qwen-image-3(?:[.-]|$)/i.test(modelId)
      ? new DashScopeImageModel(modelId, { ...options, baseURL })
