...
describe('createStreamingTextProjection', () => {
  ...
  it('keeps the streaming projection bounded for a large open code block', () => {
    const content = ['```html', ...Array.from({ length: 10_000 }, (_, index) => `<div>row ${index}</div>`)].join('\n')

    const projected = createStreamingTextProjection(content, formatProgress)

    expect(projected).toMatch(/^Generating html · 10,000 lines · [\d,]+ chars$/)
    expect(projected.length).toBeLessThan(80)
    expect(projected).not.toContain('row 9999')
  })
})
