...
const SidebarIconsManager: FC<SidebarIconsManagerProps> = ({
  ...
  const safeVisibleIcons = filterSidebarIconsForCurrentMode(visibleIcons)
  const safeDisabledIcons = filterSidebarIconsForCurrentMode(disabledIcons)
  ...
  const onDragEnd = useCallback(
    (result: DropResult) => {
      ...
      const draggedItem =
        source.droppableId === 'visible' ? safeVisibleIcons[source.index] : safeDisabledIcons[source.index]
      ...
      if (source.droppableId === destination.droppableId) {
        const list = source.droppableId === 'visible' ? [...safeVisibleIcons] : [...safeDisabledIcons]
        const [removed] = list.splice(source.index, 1)
        list.splice(destination.index, 0, removed)

        if (source.droppableId === 'visible') {
          setVisibleIcons(list)
          dispatch(setSidebarIcons({ visible: list, disabled: safeDisabledIcons }))
        } else {
          setDisabledIcons(list)
          dispatch(setSidebarIcons({ visible: safeVisibleIcons, disabled: list }))
        }
        return
      }

      const sourceList = source.droppableId === 'visible' ? [...safeVisibleIcons] : [...safeDisabledIcons]
      const destList = destination.droppableId === 'visible' ? [...safeVisibleIcons] : [...safeDisabledIcons]
    ...
    [safeVisibleIcons, safeDisabledIcons, dispatch, setVisibleIcons, setDisabledIcons, t]
  )
  ...
  const onMoveIcon = useCallback(
    (icon: SidebarIcon, fromList: 'visible' | 'disabled') => {
      ...
      if (fromList === 'visible') {
        const newVisibleIcons = safeVisibleIcons.filter((i) => i !== icon)
        const newDisabledIcons = safeDisabledIcons.some((i) => i === icon)
          ? safeDisabledIcons
          : [...safeDisabledIcons, icon]

        setVisibleIcons(newVisibleIcons)
        setDisabledIcons(newDisabledIcons)
        dispatch(setSidebarIcons({ visible: newVisibleIcons, disabled: newDisabledIcons }))
      } else {
        const newDisabledIcons = safeDisabledIcons.filter((i) => i !== icon)
        const newVisibleIcons = safeVisibleIcons.some((i) => i === icon)
          ? safeVisibleIcons
          : [...safeVisibleIcons, icon]
    ...
    [t, safeVisibleIcons, safeDisabledIcons, setVisibleIcons, setDisabledIcons, dispatch]
  )
          ...
          <Droppable droppableId="visible">
            {(provided: DroppableProvided) => (
              <IconList ref={provided.innerRef} {...provided.droppableProps}>
                {safeVisibleIcons.map((icon, index) => (
