...
vi.mock('@renderer/hooks/useShortcuts', () => ({
  useShortcut: (name: string, callback: () => void) => {
    mocks.shortcutCallbacks.set(name, callback)
  }
}))
...
describe('AgentChat', () => {
  beforeEach(() => {
    ...
    mocks.contentSearchEnable.mockReset()
  ...
  it('opens in-chat content search with the selected text shortcut', () => {
    vi.spyOn(window, 'getSelection').mockReturnValue({ toString: () => ' selected phrase ' } as Selection)
    render(<AgentChat />)

    mocks.shortcutCallbacks.get('search_message_in_chat')?.()

    expect(mocks.contentSearchEnable).toHaveBeenCalledWith('selected phrase')
  })
})
