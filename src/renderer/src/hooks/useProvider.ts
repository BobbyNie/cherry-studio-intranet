...
const selectEnabledProviders = createSelector(selectProviders, (providers) => {
  return providers.map(normalizeProvider).filter((p) => p.enabled)
})
...
const selectAllProvidersWithFallback = createSelector(selectProviders, (providers) => providers.map(normalizeProvider))
