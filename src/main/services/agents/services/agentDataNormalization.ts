...
const normalizeTimestampValue = (value: string): string | null => {
...
const normalizeAgentTimestamps = <TRow extends AgentDataRow>(
  row: TRow,
  fallbackTimestamp: string
): AgentDataNormalizationResult<TRow> => {
  const normalizedCreatedAt = normalizeTimestampValue(row.created_at)
  const normalizedUpdatedAt = normalizeTimestampValue(row.updated_at)
  ...
  if (createdAt !== row.created_at) updates.created_at = createdAt
  if (updatedAt !== row.updated_at) updates.updated_at = updatedAt
  ...
  return {
    ...
    repair: { id: row.id, original: createRepairSnapshot(row), updates }
  }
}

const normalizeAgentMcps = <TRow extends AgentDataRow>(row: TRow): AgentDataNormalizationResult<TRow> => {
  if (row.mcps === null) return { normalizedRow: row, repair: null }
  ...
  return {
    ...
    repair: { id: row.id, original: createRepairSnapshot(row), updates: { mcps: normalizedMcps } }
  }
}
...
export const normalizeAgentDataRow = <TRow extends AgentDataRow>(
  ...
  return {
    ...
    repair: {
      ...
      updates: { ...timestampResult.repair?.updates, ...mcpResult.repair?.updates }
    }
  }
}
