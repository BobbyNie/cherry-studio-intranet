...
interface ProbeDeps {
  ...
  /** Probe a single server's connectivity; failures are isolated per server. */
 ...
 * unreachable (`false`). The existing MCP service owns connection cleanup;
 * a timed-out probe may still settle after this function returns.
 */
