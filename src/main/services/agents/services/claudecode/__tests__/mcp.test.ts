...
describe('filterReachableMcpServers', () => {
  it.each(['reject', 'throw'])('isolates a probe that can %s and retries next time', async (failure) => {
    const ids = ['healthy', 'broken']
    const checkConnectivity = vi.fn((s: MCPServer) => {
      if (s.id === 'healthy') return Promise.resolve(true)
      if (failure === 'throw') throw new Error('connection failed')
      return Promise.reject(new Error('cleanup failed'))
    })
    const deps = { listServers: async () => ids.map((id) => server(id)), checkConnectivity }

    await expect(filterReachableMcpServers(ids, deps)).resolves.toEqual(['healthy'])
    checkConnectivity.mockResolvedValue(true)
    await expect(filterReachableMcpServers(ids, deps)).resolves.toEqual(ids)
    expect(ids).toEqual(['healthy', 'broken'])
  })
